...
# Installer for the SPI extension on NVIDIA JetPack 6 (Ubuntu 22.04 / aarch64).
# - Ensures linux-libc-dev is present for <linux/spi/spidev.h>
# - Loads the spidev kernel module if not already active
...
header() {
    echo "=========================================="
    echo " SPI Extension Installer (JetPack 6)"
    echo "=========================================="
    echo ""
}
...
# Ensure <linux/spi/spidev.h> is available. On JetPack 6 the Jetson kernel headers
# ship via linux-libc-dev; the standard Ubuntu package provides the uAPI.
if ! [ -f /usr/include/linux/spi/spidev.h ]; then
    step "   Installing linux-libc-dev for <linux/spi/spidev.h>..."
    $SUDO apt-get install -y --no-install-recommends linux-libc-dev
fi
ok "linux/spi/spidev.h present"

# Ensure the spidev kernel module is loaded.
if ! lsmod 2>/dev/null | grep -q '^spidev'; then
    step "   Loading spidev kernel module..."
    $SUDO modprobe spidev || true
fi
if lsmod 2>/dev/null | grep -q '^spidev'; then
    ok "spidev module loaded"
else
    echo "   ⚠️  Could not load spidev module -- load manually with: sudo modprobe spidev"
fi
